...
from repositories.junior_account import create_junior_account , list_junior_accounts , delete_junior_account , search_junior__account_by_biirth_certificate_number , get_one_junior_account , update_juinior_account_data
class JuniorAccount:
 def add_junior_account_service(self ,data):
    clean_data , error , status = validate_junior_account_data(data)
    if error:
        return error , status
    success = create_junior_account(clean_data["account_number"] , clean_data["birth_certificate_number"] , clean_data["guardian_id"], clean_data["name"] , clean_data["age"] , clean_data["amount"] , clean_data["next_of_keen"], clean_data["account_type"])
    if not success:
 ...
 def update_junior_account_service(self ,data, account_number):
    ...
    if error :
        return error , status
    success = update_juinior_account_data(clean_data["guardian_id"] , clean_data["name"] , clean_data["age"], clean_data["amount"], clean_data["next_of_keen"], account_number)
    if not success:
